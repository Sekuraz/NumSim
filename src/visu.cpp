...
void Renderer::Init(const index_t& width, const index_t& height, const int& idx, const multi_index_t &t_idx, const multi_index_t &t_dim) {
    ...
    // 100 and 50 is distance to left and top window border, here set to fit Ubuntu Unity launcher
    SDL_SetWindowPosition(_window, 100 + _width * t_idx[0], 50 + (t_dim[1] - t_idx[1] - 1) * _height);
}
//------------------------------------------------------------------------------
